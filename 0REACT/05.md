# REACT | Day No. 5 | React Basics | Date: 2025-12-30

## tags: hooks | states | conditional rendering | destructuring | declarative vs imperative
...
- ### Example of Using Rest Operator with Destructuring:
  Rest operator (`...`) can be used with destructuring to collect the remaining elements of an array or properties of an object into a new array or object.
...
#### In the following is the most important syntax to remember as it will be used more than the rest:

- The "Must-Knows":

  - Basic Array Destructuring: `const [count, setCount] = useState(0);`

  - Basic Object Destructuring: `const { name, age } = props;`

                    - Notice that destructuring an object needs the variable names to match the property names in the object.
                    - if not, the variable will be undefined.
                    - so you cant give it any name you want. unless you use the renaming syntax explained below (Renaming)

  - The Rest Operator: `const { name, ...others } = user;` (To separate one piece of data from the rest).
  - Renaming: `const { name: userName } = user;` (Useful when you already have a variable named name).

- "Good to Know"

  - Default Values: `const { theme = 'light' } = props;` (Very useful for making sure your app doesn't break if a prop is missing).

  - Skipping Items:` const [first, , third] = array;` (Common when using certain libraries that return data in specific array slots).
